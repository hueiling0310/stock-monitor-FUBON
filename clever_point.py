...
def check_clever_point(ctx: SignalContext) -> SignalResult:
    ...
    # 成交量為 0（或缺值）代表今日成交量資料不明/無效（例如即時監控情境下尚未取得
    # 真實當日累計成交量），不能直接當作「量縮」來判定成立，必須明確排除，
    # 否則 0 必然小於任何正數門檻，會被誤判成「已經量縮」。
    if pd.isna(today["Volume"]) or today["Volume"] <= 0:
        return SignalResult(
            hit=False,
            detail=f"{ctx.scan_date} 今日成交量資料無效或為0，無法判定是否量縮，不成立",
        )
